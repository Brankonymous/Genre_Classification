...
class_names=['Classical','Folk','House','Jazz','RnB','Rock']

# Ocitavanje fajla
with open('LPCC Method/lpc_features.csv') as csv_file:
    ...
    for row in csv_reader:
        ...
        if (line_cnt % 300 >=255):
            test_cnt += 1
            for i in range(0,k):
                x_test = np.append(x_test,float(row[i+1])*35)
            y_test = np.append(y_test,m[row[0]])
                
        else:
            train_cnt += 1
            for i in range(0,k):
                X_train = np.append(X_train,float(row[i+1])*35)
...
y_test = np.reshape(y_test, (test_cnt, 6))
testing = np.reshape(Y_train, (train_cnt, 6))
...
model.fit(X_train, Y_train, epochs = 150, callbacks = [es],  validation_data = (X_val, Y_val), shuffle = True)
...
correct_answer = correct_answer / 270 * 100
...
plt.title('LPC Method: Confusion Matrix of Test Data \n Precision of Test Data: ' + str(correct_answer) + '% \n')

plt.figure(2)
plot_confusion_matrix(conf_mat = train_cm, figsize = (10,10), class_names=class_names, colorbar=True)
plt.title('LPC Method: Confusion Matrix of Train Data \n')

plt.figure(3)
plot_confusion_matrix(conf_mat = val_cm, figsize = (10,10), class_names=class_names, colorbar=True)
plt.title('LPC Method: Confusion Matrix of Validation Data \n')
